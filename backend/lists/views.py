...
class TickViewSet(viewsets.ModelViewSet):
    ...
    def statistics(self, request):
        ...
        ticks = self.get_queryset().select_related('problem', 'problem__crag', 'problem__crag__city')
        ...
        # Date statistics
        date_stats = ticks.aggregate(
            first_send=Min('date'),
            latest_send=Max('date')
        )
        
        # Grade statistics
        grade_counts = Counter(tick.problem.grade for tick in ticks if tick.problem.grade)
        grade_distribution = {grade: grade_counts.get(grade, 0) for grade in GRADE_ORDER if grade in grade_counts}
        ...
        if crag_counts:
            most_climbed_crag_id, most_climbed_crag_name = crag_counts.most_common(1)[0][0]
            most_climbed_crag = {
        ...
        if city_counts:
            most_climbed_city_id, most_climbed_city_name = city_counts.most_common(1)[0][0]
            most_climbed_city = {
        ...
        # Unique locations
        unique_crags = len(set(tick.problem.crag.id for tick in ticks if tick.problem.crag))
        unique_cities = len(set(
        ...
        # Rating statistics
        ratings_with_values = [float(tick.rating) for tick in ticks if tick.rating is not None]
        average_rating = sum(ratings_with_values) / len(ratings_with_values) if ratings_with_values else None
        rated_problems_count = len(ratings_with_values)
        
        # Activity by year
        ticks_by_year = Counter(
            tick.date.year for tick in ticks if tick.date
        )
        ticks_per_year = {str(year): count for year, count in sorted(ticks_by_year.items())}
        most_active_year = None
        if ticks_by_year:
            most_active_year = max(ticks_by_year.items(), key=lambda x: x[1])
            most_active_year = {"year": most_active_year[0], "tick_count": most_active_year[1]}
        
        # Activity by month (across all years)
        ticks_by_month = Counter(
            tick.date.month for tick in ticks if tick.date
        )
        month_names = {
        ...
        ticks_per_month = {month_names[month]: count for month, count in sorted(ticks_by_month.items())}
        ...
        return Response({
            ...
            "first_send": date_stats['first_send'].isoformat() if date_stats['first_send'] else None,
            "latest_send": date_stats['latest_send'].isoformat() if date_stats['latest_send'] else None,
