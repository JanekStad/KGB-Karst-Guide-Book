...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for item in data:
            ...
            if not problem_name:
                self.stdout.write(self.style.WARNING(f'Skipping item with no name: {item}'))
                stats['problems_skipped'] += 1
                continue

            if not crag_name:
                self.stdout.write(self.style.WARNING(f'Skipping "{problem_name}" - no crag name'))
                stats['problems_skipped'] += 1
                continue
            ...
            crag, created = Crag.objects.get_or_create(
                ...
                defaults={
                    ...
                    'description': f'Located in {city}, {location}' if city or location else '',
                    'created_by': user,
                }
            )
            if created:
            ...
            if wall_name:
                ...
                if created:
                    ...
                    self.stdout.write(self.style.SUCCESS(f'Created wall: {crag_name} - {wall_name}'))
                else:
            ...
            if not dry_run:
                ...
                if exists:
                    stats['problems_existing'] += 1
                    self.stdout.write(f'Would skip (exists): {problem_name} at {crag_name}')
                else:
                    stats['problems_created'] += 1
                    self.stdout.write(f'Would create: {problem_name} ({grade}) at {crag_name}')
