...
def detect_labels(
    img: np.ndarray, min_width: int = 40, min_height: int = 15
) -> Dict[str, Dict]:
    """
...
def add_text_label(
    img: np.ndarray, text: str, position: Tuple[int, int] = (10, 30)
) -> np.ndarray:
    ...
    cv2.putText(
        img_copy, text, position, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1, cv2.LINE_AA
    )
    return img_copy
...
def detect_lines(
    ...
    if debug_output:
        ...
        if pixel_count == 0:
            ...
            print(
                "     Try different --preset values or adjust --lower-hsv/--upper-hsv"
            )
...
def extract_normalized_coordinates(
    skel: np.ndarray, height: int, width: int
) -> np.ndarray:
    """
...
def cluster_lines(
    norm_points: np.ndarray, eps: float = 0.01, min_samples: int = 10
...
def assign_routes_to_labels(
    ...
    for rlab in unique_routes:
        ...
        if best_name:
            ...
            route_map[best_name] = [{"x": float(x), "y": float(y)} for y, x in ordered]
...
def sample_line_colors(
    img: np.ndarray, num_samples: int = 20
    ...
    try:
        ...
        if args.sample_colors:
            ...
            if lower is not None and upper is not None:
                ...
                print(
                    "Could not sample colors. Try using a preset or manual HSV values."
                )
            return
        ...
        if np.sum(mask > 0) == 0:
            ...
            print("\nTry:")
            print(
                "  1. Use a preset: --preset orange-bright (or orange-dark, orange-light)"
            )
        ...
        route_labels, unique_routes = cluster_lines(
            norm_points, eps=args.eps, min_samples=args.min_samples
        )

        if len(unique_routes) == 0:
            print(
                "Warning: No routes clustered. Try adjusting --eps and --min-samples parameters."
            )
            route_map = {}
        else:
            # Assign routes to labels
            route_map = assign_routes_to_labels(
                norm_points, route_labels, unique_routes, labels, h, w
            )
